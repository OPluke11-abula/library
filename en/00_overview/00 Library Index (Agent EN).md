---
tags:
  - library
  - index-navigator
  - agent-optimized
created: 2026-09-17
updated: 2026-09-22
author: Luke
---

> 🌐 **Language / 語言**: [🇹🇼 繁體中文 (Traditional Chinese)](../../00%20%E5%9C%96%E6%9B%B8%E9%A4%A8%E7%B8%BD%E8%A6%BD%20%28Library%20Index%29.md) | 🇺🇸 **English (Agent Edition)**

# CSIE & Deep Learning Grand Library (Agent Overview & Master Index)

> 🏛️ **Agent Knowledge System Architecture**:
> This repository implements a formal, 10-Wing Directed Acyclic Graph (DAG) grounded in ACM Computing Classification System (CCS) standards, peer-reviewed AI literature (CVPR, ICCV, NeurIPS, ICML, ICLR), and NVIDIA DLI accelerated computing architectures.

Primary Master Topological Entry Point:
👉 **[[LIB-000 Grand Library Index & Navigator (Agent EN)]]**

---

### The 10 Specialized Wings & Flagship Volumes (Agent Edition)
- **00 Overview & Topological Navigation**:
  - [[LIB-000 Grand Library Index & Navigator (Agent EN)]]
  - [[LIB-001 Deep Learning First Principles (Agent EN)]]
- **01 Mathematical & Theoretical Foundations**:
  - [[LIB-101 Linear Algebra & High-Dimensional Geometry (Agent EN)]]
  - [[LIB-104 Convex Optimization & Gradient Descent (Agent EN)]]
- **02 Computer Systems & Hardware Alignment**:
  - [[LIB-203 Computer Architecture & Hardware-Aware Deep Learning (Agent EN)]]
- **03 Machine Learning & Statistical Principles**:
  - [[LIB-301 Dataset Bias, Domain Shift & Spatial Penalties (Agent EN)]]
- **04 Deep Learning Architectures & Neural Mechanisms**:
  - [[LIB-401 DNN Spatial Limits & CNN Inductive Bias (Agent EN)]]
  - [[LIB-405 Attention Mechanism & Transformer Revolution (Agent EN)]]
  - [[LIB-406 Generative Frontiers - SDE Diffusion to Flow Matching (Agent EN)]]
- **05 Computer Vision & High-Dimensional Sensing**:
  - [[LIB-501 CV Preprocessing & Center of Mass Alignment (Agent EN)]]
  - [[LIB-504 3D Gaussian Splatting Theory & Rasterization (Agent EN)]]
- **06 NLP & Large Language Models**:
  - [[LIB-602 Modern LLM Architecture & Scaling Laws (Agent EN)]]
- **07 Reinforcement Learning & Intelligent Agents**:
  - [[LIB-703 LLM Agent Cognitive Architecture & Protocols (Agent EN)]]
  - [[LIB-704 Dual-Process Neural Agent S1-Jev & Reflex CUA-S1 (Agent EN)]]
- **08 AI Systems Engineering & High-Efficiency Deployment**:
  - [[LIB-801 Model Calibration & Uncertainty Estimation (Agent EN)]]
  - [[LIB-802 Quantization Mathematics & Low-Precision Inference (Agent EN)]]
- **09 Research Methodology & Capstone Blueprints**:
  - [[LIB-901 Classic Project Post-Mortem - Production MNIST (Agent EN)]]
  - [[LIB-903 Capstone Blueprint & Academic Research Evolution (Agent EN)]]
  - [[LIB-904 Academic Research Corpus & Literature Synthesis (Agent EN)]]
